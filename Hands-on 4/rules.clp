...
(defrule rule-4
  ...
  (orden (part-number ?smartphone-part-number) (tipo "smartphone"))
...
(defrule identificar-mayorista
  (orden (qty ?quantity) (part-number ?pid))
  (test (> ?quantity 9))
...
(defrule identificar-minorista
  ...
  =>
  (retract ?customer)
  (assert (customer (customer-id ?custId) (kind-of-client "Minorista")))
  (assert (order (order-number ?orNum) (customer-id ?custId)))
  (printout t "El cliente con ID " ?custId " es identificado como Minorista." crlf)
)
...
(defrule rule-9
    ...
    (printout t "El cliente con ID " ?cust-id " ha recibido un  de $50 por la compra de un iPhone 16." crlf)
)

;descuento por comprar un 1 iphone 16
(defrule rule-10
    (order (order-number ?order-num) (customer-id ?cust-id))
    (line-item (order-number ?order-num) (part-number ?part-num))
    (orden (part-number ?part-num) (modelo "iPhone 16") )
...
(defrule rule-11-meses-sin-intereses
  ...
  => 
  (printout t "tiene 3 meses sin intereses" ?grcred crlf)  
)

;oferta 
(defrule rule-12 
  (orden (part-number ?numPa)(tipo "smartphone")(marca "apple")(modelo "iPhone 16")(qty ?qt))
  => 
  (printout t "agrega un mac Laptop modelo  MacBook Air y con tu numero de cliente obten un vale de regalo" crlf)   
)
...
(defrule rule-15
  (orden (modelo "iPhone 16") (part-number ?pd) (qty ?quty))
  ...
  ; Imprimir mensaje confirmando el envío gratuito
...
(defrule rule-19-bono
   (orden (tipo ?tipo) (marca ?marca) (modelo ?modelo) (qty ?cantidad))
   (line-item (part-number ?pd) (customer-id ?cust-id))  
   (test (>= ?cantidad 10))
   =>
...
(defrule actualizar-stock-smartphone
  ?orden <- (orden (part-number ?part-number) (tipo "smartphone") (marca ?marca) (modelo ?modelo) (qty ?cantidad))
  ?producto <- (smartphone (part-number ?part-number) (marca ?marca) (modelo ?modelo) (qty ?stock-cant))
  (not (stock-actualizado (part-number ?part-number)))
  =>
  (retract ?producto)
  (assert (smartphone (part-number ?part-number) (marca ?marca) (modelo ?modelo) (qty (- ?stock-cant ?cantidad))))
  (assert (stock-actualizado (part-number ?part-number)))
...
(defrule actualizar-stock-computadoras
  ?orden <- (orden (part-number ?part-number) (tipo "computadora") (marca ?marca) (modelo ?modelo) (qty ?cantidad))
...
(defrule actualizar-stock-accesorios
  ?orden <- (orden (part-number ?part-number) (tipo "accesorio") (marca ?nombre) (qty ?cantidad))
...
(defrule imprimir-stock-smartphones
  ?producto <- (smartphone (part-number ?part-number) (marca ?marca) (modelo ?modelo) (qty ?stock-cant))
  =>
  (printout t "Stock actual para " ?marca " " ?modelo ": " ?stock-cant " unidades." crlf)
)

(defrule imprimir-stock-computadoras
  ?producto <- (computadoras (part-number ?part-number) (marca ?marca) (modelo ?modelo) (qty ?stock-cant))
  =>
  (printout t "Stock actual para " ?marca " " ?modelo ": " ?stock-cant " unidades." crlf)
)

(defrule imprimir-stock-accesorios
  ?producto <- (accesorios (part-number ?part-number) (nombre ?nombre) (qty ?stock-cant))
  =>
  (printout t "Stock actual para " ?nombre ": " ?stock-cant " unidades." crlf)
)